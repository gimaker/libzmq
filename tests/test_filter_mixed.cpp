...
{
    fprintf (stderr, "test_filter_mixed running...\n");
    ...
    //  Subscribe to "a" using prefix filering
    rc = zmq_setsockopt (sub, ZMQ_SUBSCRIBE, "a", 1);
    assert (rc == 0);

    //  Subscribe to "b" using exact matching
    ...
    //  Wait a bit till the subscriptions gets to the publisher.
    zmq_sleep (1);
    ...
    // 4. Send a matching (exact) message
    sprintf (sndbuff, "b");
    rc = zmq_send (pub, sndbuff, 1, 0);
    assert (rc == 1);
    ...
    // 5. Send a non-matching message
    sprintf (sndbuff, "beef");
    rc = zmq_send (pub, sndbuff, 4, 0);
    assert (rc == 4);
